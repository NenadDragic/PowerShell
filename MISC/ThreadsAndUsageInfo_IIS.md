# IIS w3wp Threads & Usage Info - ThreadsAndUsageInfo_IIS.ps1

This script collects per-`w3wp.exe`-process resource usage (threads, handles, CPU time, memory, owning user) and its associated IIS app pool/site info from a list of remote servers, saving one timestamped CSV per server.

## How it works

1. Reads the target server list from `$Servers` (fill in machine names before use) and optionally prompts for alternate credentials if `$UseCredential` is `$true`.
2. Creates the local output directory `C:\Temp\w3wp-logs` and a shared timestamp for the whole run.
3. For each server, runs a remote script block via `Invoke-Command` that:
   - Imports `WebAdministration` if available.
   - Gets all `w3wp.exe` processes via WMI (`Get-WmiObject Win32_Process`).
   - Gets IIS site-to-app-pool mappings via `Get-WebConfiguration`, if the module is present.
   - For each `w3wp` process, extracts the app pool name from its command line (`-ap "<name>"`), memory usage, owning user (via `GetOwner()`), and thread/handle counts and CPU time (via `Get-Process`, falling back to WMI properties if that fails).
   - Matches the app pool to its site name(s).
   - Builds a result row per process with date, time, computer, app pool, sites, PID, threads, handles, CPU time, memory (MB), and user.
4. Exports each server's results to its own CSV (`<server>_<timestamp>.csv`) in the output directory.
5. Combines all servers' results and prints them as a table at the end. A commented-out block shows how to also export one combined CSV across all servers.

## Usage

Fill in `$Servers` with real machine names before running:

```powershell
.\ThreadsAndUsageInfo_IIS.ps1
```

Set `$UseCredential = $true` if you need to run under alternate credentials — you'll be prompted via `Get-Credential`.
