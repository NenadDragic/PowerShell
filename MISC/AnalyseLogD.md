# Analyse Log Directory Sizes - AnalyseLogD.ps1

This script recursively scans every directory under `D:\`, measures the total size and count of `.log` files in each, flags directories whose oldest log file is older than a configurable age threshold, and writes a report to file and screen.

## How it works

1. Accepts `-months` (default `1`), the age threshold in months for flagging "old" log directories.
2. Recursively lists every subdirectory under `D:\`.
3. For each directory containing `.log` files, computes: file count, total size (GB), oldest/newest file creation date, whether the oldest file exceeds the age threshold (marked with a checkmark), and the count/size of files older than the threshold.
4. Accumulates running totals (log file count, old file count, total size, old file size) across all directories.
5. Prints a table of per-directory results, followed by a summary block.
6. Saves the table plus summary to `C:\Temp\<computername>.txt`.
7. Prints the summary totals to the console.

## Usage

```powershell
.\AnalyseLogD.ps1 -months 3
```

Adjust `$outputDirectory` in the script if you want the report saved somewhere other than `C:\Temp\`.
